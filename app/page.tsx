...
export default async function HomePage(){
  ...
  while(socialImages.length<4&&hero)socialImages.push(hero);

  return <main className="pt-[124px] md:pt-[102px]" style={{background:settings.page_bg_color,color:settings.text_color}}>
    <section className="relative h-[calc(100svh-124px)] min-h-[520px] md:h-[calc(100svh-102px)] overflow-hidden bg-[var(--product-card-bg)]">
      ...
      <div className="relative z-10 h-full flex items-center justify-center text-center px-5">
        <div>
          {settings.hero_eyebrow&&<p className="kicker !text-white/75 mb-5">{settings.hero_eyebrow}</p>}
    ...
    <section className="site-shell py-10 lg:py-14">
      <div className="mb-8 md:flex md:items-end md:justify-between md:gap-8">
        <div>
          <h2 className="text-[20px] font-normal tracking-[.01em] mb-2">{settings.best_sellers_title}</h2>
          {settings.best_sellers_subtitle&&<p className="text-[14px] md:text-[15px] text-black/55 max-w-[620px] leading-7">{settings.best_sellers_subtitle}</p>}
        </div>
        <Link href="/shop" className="hidden md:inline-flex text-[14px] text-black/65 hover:text-black underline-offset-4 hover:underline">Shop all</Link>
      </div>
      {products.length
        ?<div className="grid grid-cols-2 lg:grid-cols-4 gap-x-3 gap-y-11">{products.slice(0,8).map((p,i)=><ProductCard key={p.id} product={p} index={i}/>)}</div>
        :<div className="product-bg py-20 text-center"><h3 className="text-3xl font-light">No products yet</h3><p className="mt-3 text-black/55">Add your first products from the admin area.</p></div>}
    </section>
    ...
    <section id="about" className="bg-white">
      <div className="relative grid md:grid-cols-2">
        ...
        <Link href={settings.purpose_button_url||`mailto:${settings.contact_email}`} className="contact-overlay-btn">
      ...
      <div className="site-shell py-12 lg:py-16 text-center">
        ...
        <div className="social-grid mt-8">
          {socialImages.slice(0,4).map((img,idx)=><div key={`${img}-${idx}`} className="social-grid-item"><SmartImage src={img} alt={`Balisa detail ${idx+1}`}/></div>)}
        </div>
      </div>
    </section>
    ...
    <section className="site-shell py-3 lg:py-6">
      <NewsletterForm
        ...
        text={settings.newsletter_text||'Be the first to know about new collections and get early access.'}
